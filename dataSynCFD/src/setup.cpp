...
void main_setup() { // water stirring; required extensions: FP16S, MOVING_BOUNDARIES, SUBGRID, INTERACTIVE_GRAPHICS or GRAPHICS, VOLUME_FORCE, EQUILIBRIUM_BOUNDARIES, SURFACE
	...
	const uint3 lbm_N = resolution(float3(3.0f, 3.0f, 1.0f), 181u); // Simulation domain resolution
	const float lbm_Re = 1000.0f;
	const float lbm_u = 0.1f;
	const float lbm_mu = 0.01f;
	const float lbm_rho = 1000.0f;
	const ulong lbm_T = 48000ull;
	const ulong lbm_dt = 10ull;
	const float f = 0.001f; // Gravity effect
 
	LBM lbm(lbm_N, 1u, 1u, 1u, units.nu_from_Re(lbm_Re, (float)lbm_N.x, lbm_u), 0.0f, 0.0f, -f); // Apply gravity
	//LBM lbm(lbm_N, 1u, 1u, 1u, units.nu_from_mu(lbm_mu, lbm_rho), 0.0f, 0.0f, -f); // Apply gravity
	// ###################################################################################### define geometry ######################################################################################
	...
	parallel_for(lbm.get_N(), [&](ulong n) {
	   ...
	   // Define water region
	   if (z < H) {
		  lbm.flags[n] = TYPE_F; // Fluid
		  lbm.rho[n] = units.rho_hydrostatic(f, (float)z, (float)H); // Hydrostatic pressure
	   }
 
	   // Define solid boundaries
	   if (x == 0u || x == Nx - 1u || y == 0u || y == Ny - 1u || z == 0u)lbm.flags[n] = TYPE_S; // Non-periodic boundary
	   });
 
	// ####################################################################### run simulation, export images and data ##########################################################################
	lbm.graphics.visualization_modes = VIS_PHI_RAYTRACE | VIS_Q_CRITERION ; //VIS_FLAG_LATTICE | VIS_FLAG_SURFACE; //| VIS_Q_CRITERION;
	lbm.run(0u, lbm_T); // Initialize simulation
 
	while (lbm.get_t() < lbm_T) { // Main simulation loop
	   lbm.voxelize_mesh_on_device(mesh, TYPE_S, center, float3(0.0f), float3(0.0f, 0.0f, lbm_omega)); // Rotate fan in water
	   lbm.run(lbm_dt, lbm_T);
	   mesh->rotate(float3x3(float3(5.0f, 0.0f, 0.0f), lbm_domega)); // Rotate mesh
